...
class Model():
    ...
    def generative_model(self, batch, additional_inputs_tf):
        ...
        self.gen_prior_latent_code_logpdf = self.prior_dist.log_pdf(self.gen_prior_latent_code)
        self.gen_prior_logpdf = tf.reduce_mean(self.gen_prior_latent_code_logpdf)
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        self.prior_latent_code_logpdf = self.prior_dist.log_pdf(self.prior_latent_code)
        ...
        self.epsilon_param = self.EpsilonMap.forward((tf.zeros(shape=(batch_size_tf, 1)),))
        ...
        if b_stochastic_encoder: self.posterior_latent_code_expanded = self.EncodingPlan.forward(self.input_sample, epsilon_sample=self.epsilon)
        else: self.posterior_latent_code_expanded = self.EncodingPlan.forward(self.input_sample)
        ...
        self.posterior_logpdf = tf.reduce_mean(self.posterior_latent_code_logpdf)
        self.reg_target_param = self.Decoder.forward(self.posterior_latent_code_expanded) 
        self.reg_target_dist = distributions.ProductDistribution(sample_properties = batch['observed']['properties'], params = self.reg_target_param)
        ...
        self.critic_reg_target = self.Discriminator.forward(self.reg_target_sample)

        self.real_reg_distances_sq = self.metric_distance_sq(self.input_sample, self.reg_target_sample)
        # self.real_reg_slopes_sq = (self.critic_real-self.critic_reg_target)/(self.real_reg_distances_sq+1e-7)
        self.real_reg_slopes_sq = (self.critic_real-self.critic_reg_target)/(helper.safe_tf_sqrt(self.real_reg_distances_sq)+1e-7)

        try:
            self.convex_grad = tf.gradients(self.critic_reg_target, [self.reg_target_sample['image']])[0]
            self.convex_grad_norm = helper.safe_tf_sqrt(tf.reduce_sum(self.convex_grad**2, axis=[-1,-2,-3], keep_dims=False)[:,:,np.newaxis])
        except: 
            self.convex_grad = tf.gradients(self.critic_reg_target, [self.reg_target_sample['flat']])[0]
        ...
        self.mean_critic_reg = tf.reduce_mean(self.critic_reg_target)
