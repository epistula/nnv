...
class Model():
    ...
    def rbf_kernel(self, z_1, z_2, sigma_z_sq=1):
        z_diff = z_1-z_2
        z_diff_norm_sq = tf.reduce_sum(z_diff**2, axis=[-1], keep_dims=True)
        n_dim = z_diff.get_shape().as_list()[-1]
        sigma_k_sq = 2*n_dim*sigma_z_sq
        return tf.exp(-z_diff_norm_sq/sigma_k_sq)

    def inv_multiquadratics_kernel(self, z_1, z_2, sigma_z_sq=1):
        z_diff = z_1-z_2
        z_diff_norm_sq = tf.reduce_sum(z_diff**2, axis=[-1], keep_dims=True)
        n_dim = z_diff.get_shape().as_list()[-1]
