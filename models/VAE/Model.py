...
import models.VAE.ModelMapsDCGAN as ModelMaps
...
class Model():
    ...
    def generative_model(self, batch, additional_inputs_tf):
        ...
        if len(properties_dirac_dict['flat'])>0:
            for e in properties_dirac_dict['flat']: e['dist']='dirac'
        else:
            for e in properties_dirac_dict['image']: e['dist']='dirac'

        self.gen_input_sample = batch['observed']['data']
        self.gen_input_dist = distributions.ProductDistribution(sample_properties = properties_dirac_dict, params = self.gen_input_sample)
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        self.input_dist = distributions.ProductDistribution(sample_properties = properties_dirac_dict, params = self.input_sample)
        ...
        self.obs_sample_dist_pre = distributions.ProductDistribution(sample_properties = batch['observed']['properties'], params = self.obs_sample_param)
        self.obs_sample = self.obs_sample_dist_pre.sample()
        self.obs_sample_dist = distributions.ProductDistribution(sample_properties = properties_dirac_dict, params = self.obs_sample)
        ...
        self.reg_dist = distributions.ProductDistribution(sample_properties = properties_dirac_dict, params = self.reconst_sample_noised)
