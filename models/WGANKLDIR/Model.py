...
class Model():
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        try: 
            ...
            self.convex_slope_vector = self.input_sample['image']-self.reg_target_sample['image']
            self.convex_slope_dir = self.convex_slope_vector/helper.safe_tf_sqrt(tf.reduce_sum(self.convex_slope_vector**2, axis=[-1,-2,-3], keep_dims=False)[:,:,np.newaxis])
        except: 
            ...
            self.convex_slope_vector = self.input_sample['flat']-self.reg_target_sample['flat']
            self.convex_slope_dir = self.convex_slope_vector/helper.safe_tf_sqrt(tf.reduce_sum(self.convex_grad**2, axis=[-1], keep_dims=True))
        ...
        try:
            ...
            self.convex_grad_dir = self.convex_grad/self.convex_grad_norm
        except: 
        ...
        self.gradient_dir_penalties = self.cosine_distance(self.convex_slope_dir, self.convex_grad_dir)
        ...
        self.mean_gradient_dir_penalty = tf.reduce_mean(self.gradient_dir_penalties)

        self.regularizer_cost = 10*self.mean_gradient_penalty
        self.regularizer_cost2 = 10*self.mean_gradient_dir_penalty
        self.discriminator_cost = -self.mean_critic_real+self.mean_critic_gen+self.regularizer_cost+self.regularizer_cost2
