# BASED ON https://github.com/martinarjovsky/WassersteinGAN/blob/master/models/dcgan.py
...
class EncoderSampler():
	def __init__(self, config, name = '/EncoderSampler'):
		self.name = name
		self.config = config
		self.activation_function = activation_function
		self.constructed = False
	...
	def forward(self, x, noise=None, name = ''):
		with tf.variable_scope("EncoderSampler", reuse=self.constructed):
			if len(self.config['data_properties']['flat']) > 0:
				n_output_size = helper.list_sum([distributions.DistributionsAKA[e['dist']].num_params(e['size'][-1]) for e in self.config['data_properties']['flat']])				
				x_batched_inp_flat = tf.reshape(x['flat'], [-1,  *x['flat'].get_shape().as_list()[2:]])
			...
			if len(self.config['data_properties']['image']) > 0:								
				...
				if image_shape == (64, 64):
					n_filters = 64
					lay1_image = tf.layers.conv2d(inputs=x_batched_inp_image, filters=n_filters, kernel_size=[5, 5], strides=[2, 2], padding="valid", activation=activation_function)
					lay2_image = tf.layers.conv2d(inputs=lay1_image, filters=n_filters, kernel_size=[5, 5], strides=[2, 2], padding="valid", activation=activation_function)
					lay3_image = tf.layers.conv2d(inputs=lay2_image, filters=2*n_filters, kernel_size=[5, 5], strides=[1, 1], padding="valid", activation=activation_function)
					lay4_image = tf.layers.conv2d(inputs=lay3_image, filters=3*n_filters, kernel_size=[5, 5], strides=[1, 1], padding="valid", activation=activation_function)
					latent_image = tf.layers.conv2d(inputs=lay4_image, filters=3*n_filters, kernel_size=[3, 3], strides=[1, 1], padding="valid", activation=activation_function)
					latent_image_flat = tf.reshape(latent_image, [-1, np.prod(latent_image.get_shape().as_list()[1:])])
				...
				if noise is not None:
					lay1_flat = tf.layers.dense(inputs = tf.concat([latent_image_flat, noise],axis=-1), units = self.config['n_encoder'], activation = activation_function)
				else:
					lay1_flat = tf.layers.dense(inputs = latent_image_flat, units = self.config['n_encoder'], activation = activation_function)
	...
	def forward(self, x, name = ''):
		with tf.variable_scope("Decoder", reuse=self.constructed):
			...
			if len(self.config['data_properties']['image']) > 0:
				...
				# lay3_flat = tf.layers.dense(inputs = lay2_flat, units = self.config['n_decoder'], activation = activation_function)
				# lay4_flat = tf.layers.dense(inputs = lay3_flat, units = self.config['n_decoder'], activation = activation_function)
				# image_param_flat = tf.layers.dense(inputs = lay4_flat, units = n_output_channels*n_image_size, activation = tf.nn.sigmoid)
				...
				if image_shape == (28, 28):
					...
					image_param = tf.nn.sigmoid(lay10_image)

				# 64x64xn_channels
				if image_shape == (64, 64):
					b_batch_norm = True
					n_filters = 64
					lay1_image = x_batched_inp_flat[:, np.newaxis, np.newaxis, :]
					
					lay2_image = tf.layers.conv2d_transpose(inputs=lay1_image, filters=4*n_filters, kernel_size=[4, 4], strides=[1, 1], activation=None, use_bias=False)
					if b_batch_norm: lay3_image = tf.nn.relu(helper.batch_norm()(lay2_image))
					else: lay3_image = tf.nn.relu(lay2_image) #h1

					lay4_image = tf.layers.conv2d_transpose(inputs=lay3_image, filters=4*n_filters, kernel_size=[4, 4], strides=[2, 2], activation=None, use_bias=False)
					lay5_image = helper.tf_center_crop_image(lay4_image, resize_ratios=[8,8])
					if b_batch_norm: lay6_image = tf.nn.relu(helper.batch_norm()(lay5_image))
					else: lay6_image = tf.nn.relu(lay5_image) #h1

					lay7_image = tf.layers.conv2d_transpose(inputs=lay6_image, filters=2*n_filters, kernel_size=[4, 4], strides=[2, 2], activation=None, use_bias=False)
					lay8_image = helper.tf_center_crop_image(lay7_image, resize_ratios=[16,16])
					if b_batch_norm: lay9_image = tf.nn.relu(helper.batch_norm()(lay8_image))
					else: lay9_image = tf.nn.relu(lay8_image) #h1

					lay10_image = tf.layers.conv2d_transpose(inputs=lay9_image, filters=2*n_filters, kernel_size=[4, 4], strides=[2, 2], activation=None, use_bias=False)
					lay11_image = helper.tf_center_crop_image(lay10_image, resize_ratios=[32,32])
					if b_batch_norm: lay12_image = tf.nn.relu(helper.batch_norm()(lay11_image))
					else: lay12_image = tf.nn.relu(lay11_image) #h1

					lay13_image = tf.layers.conv2d_transpose(inputs=lay12_image, filters=1*n_filters, kernel_size=[4, 4], strides=[2, 2], activation=None, use_bias=False)
					lay14_image = helper.tf_center_crop_image(lay13_image, resize_ratios=[64,64])
					if b_batch_norm: lay15_image = tf.nn.relu(helper.batch_norm()(lay14_image))
					else: lay15_image = tf.nn.relu(lay14_image) #h1

					lay16_image = tf.layers.conv2d_transpose(inputs=lay15_image, filters=n_output_channels, kernel_size=[3, 3], strides=[1, 1], activation=None, use_bias=False)
					lay17_image = helper.tf_center_crop_image(lay16_image, resize_ratios=[64,64])
					image_param = tf.nn.sigmoid(lay17_image)
				...
				if image_shape == (128, 128):
					...
					image_param = tf.nn.sigmoid(lay16_image)
		...
		with tf.variable_scope("Discriminator", reuse=self.constructed):
			...
			if x['image'] is not None: 
				...
				if image_shape == (64, 64):
					...
					lay1_image = helper.lrelu(tf.layers.conv2d(inputs=x_batched_inp_image, filters=n_filters, kernel_size=[4, 4], strides=[2, 2], padding="same", activation=None, use_bias=False))
					
					lay2_image = tf.layers.conv2d(inputs=lay1_image, filters=2*n_filters, kernel_size=[4, 4], strides=[2, 2], padding="same", activation=None, use_bias=False)
					if b_batch_norm: lay3_image = helper.lrelu(helper.batch_norm()(lay2_image)) 
					else: lay3_image = helper.lrelu(lay2_image)

					lay4_image = tf.layers.conv2d(inputs=lay3_image, filters=4*n_filters, kernel_size=[4, 4], strides=[2, 2], padding="same", activation=None, use_bias=False)
					if b_batch_norm: lay5_image = helper.lrelu(helper.batch_norm()(lay4_image)) 
					else: lay5_image = helper.lrelu(lay4_image)

					lay6_image = tf.layers.conv2d(inputs=lay5_image, filters=8*n_filters, kernel_size=[4, 4], strides=[2, 2], padding="same", activation=None, use_bias=False)
					if b_batch_norm: lay7_image = helper.lrelu(helper.batch_norm()(lay6_image)) 
					else: lay7_image = helper.lrelu(lay6_image)

					critic_image = tf.layers.conv2d(inputs=lay7_image, filters=1, kernel_size=[4, 4], strides=[1, 1], padding="valid", activation=None, use_bias=False)
...
class DivergenceLatent():
	def __init__(self, config, name = '/DivergenceLatent'):
		self.name = name
		self.activation_function = activation_function
		self.config = config
		self.constructed = False
	...
	def forward(self, z, name = ''):
		with tf.variable_scope("DivergenceLatent", reuse=self.constructed):
			outputs = []
		
			z_batched_inp_flat = tf.reshape(z, [-1,  *z.get_shape().as_list()[2:]])
			lay1_flat = tf.layers.dense(inputs = z_batched_inp_flat, units = self.config['n_encoder'], activation = activation_function)
			lay2_flat = tf.layers.dense(inputs = lay1_flat, units = self.config['n_encoder'], activation = activation_function)
			lay3_flat = tf.layers.dense(inputs = lay2_flat, units = self.config['n_encoder'], activation = activation_function)
			# lay4_flat = tf.layers.dense(inputs = lay3_flat, units = self.config['n_encoder'], activation = activation_function)
			lay5_flat = tf.layers.dense(inputs = lay3_flat, units = self.config['n_encoder'], activation = activation_function)
			critic_flat = tf.layers.dense(inputs = lay5_flat, units = 1, activation = tf.nn.sigmoid)
			outputs.append(tf.reshape(critic_flat, [-1, 1, 1]))
