...
class Model():
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        self.WAE_based_grad = tf.gradients(self.mean_OT_primal, [self.reconst_sample['image']])[0]
        self.WGAN_based_grad = tf.gradients(-self.mean_critic_rec, [self.reconst_sample['image']])[0]
        self.WAE_WGAN_grad_dists = self.euclidean_distance_squared(self.WAE_based_grad, self.WGAN_based_grad, axis=[-1,-2,-3], keep_dims=False)[:,:,np.newaxis]
        self.WAE_WGAN_grad_dist_mean = tf.reduce_mean(self.WAE_WGAN_grad_dists)
