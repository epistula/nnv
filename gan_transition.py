...
parser.add_argument('--global_exp_dir', type=str, default='./experimentsEEE', help='Directory to put the experiments.')
...
# # # # TOY
# parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
# parser.add_argument('--global_exp_dir', type=str, default='./experimentsGanTests34', help='Directory to put the experiments.')
# parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
# parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
# parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
# parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
...
from models.GEODESIC3.Model import Model
...
with tf.Graph().as_default():
    ...
    with tf.variable_scope("training"):
        ...
        discriminator_vars = [v for v in tf.trainable_variables() if 'Discriminator' in v.name] #len=6
        geo_generator_vars = [v for v in tf.trainable_variables() if 'GeoDiff' in v.name] 
    ...
    if global_args.optimizer_class == 'RmsProp':
        train_discriminator_step_tf = tf.train.RMSPropOptimizer(learning_rate=0.0001, 
            momentum=0.9).minimize(train_outs_dict['discriminator_cost'], var_list=discriminator_vars, global_step=global_step)
        train_geo_generator_step_tf = tf.train.RMSPropOptimizer(learning_rate=0.0001, 
            momentum=0.9).minimize(train_outs_dict['geo_generator_cost'], var_list=geo_generator_vars, global_step=global_step)
        # train_generator_step_tf = tf.train.RMSPropOptimizer(learning_rate=0.0001, 
    ...
    #     train_geo_generator_step_tf = tf.train.AdamOptimizer(learning_rate=0.0001, 
    #         beta1=0.5, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['geo_generator_cost'], var_list=geo_generator_vars, global_step=global_step)
    ...
    #     #     beta1=0.5, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['generator_cost'], var_list=generator_vars, global_step=global_step)
        ...
        train_geo_generator_step_tf = tf.train.AdamOptimizer(learning_rate=0.0001, 
            beta1=0.9, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['geo_generator_cost'], var_list=geo_generator_vars, global_step=global_step)
        # train_generator_step_tf = tf.train.AdamOptimizer(learning_rate=0.0001, 
        #     beta1=0.5, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['generator_cost'], var_list=generator_vars, global_step=global_step)
    ...
    def train(epoch):
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            if batch_idx % global_args.log_interval == 0:
                ...
                print('Train: Epoch {} [{:7d} ()]\tGenerator Cost: {:.6f}\tDiscriminator Cost: {:.6f}\tTime: {:.3f}, geo_const_1 {:.6f}, geo_const_2 {:.6f}, alpha_const {:.6f}'.format(
                      epoch, batch_idx * curr_batch_size, generator_cost_np, discriminator_cost_np, (end - start), mean_geo_constraint_1_np, mean_geo_constraint_2_np, jacobian_alpha_constraint_np))
        ...
        if epoch % checkpoint_time == 0:
            ...
            if data_loader.__module__ == 'datasetLoaders.RandomManifoldDataLoader' or data_loader.__module__ == 'datasetLoaders.ToyDataLoader':
                ...
                helper.visualize_datasets2(sess, input_dict_func, data_loader, data_loader.dataset, generative_dict['interpolated_sample'],
                    save_dir=global_args.exp_dir+'Visualization/', postfix=str(epoch)) 

                helper.visualize_datasets3(sess, input_dict_func, data_loader, data_loader.dataset, generative_dict['doubly_interpolated_sample'],
                    save_dir=global_args.exp_dir+'Visualization/', postfix=str(epoch)) 
                
                helper.visualize_datasets4(sess, input_dict_func, data_loader, generative_dict['interpolated_sample_begin'], generative_dict['interpolated_sample_end'], generative_dict['interpolated_sample'],
                    save_dir=global_args.exp_dir+'Visualization/', postfix=str(epoch)) 
                ...
                batch['observed']['data']['flat'] = np.concatenate([XY[:int(XY.shape[0]/2), np.newaxis, :], XY[:, np.newaxis, :]], axis=0)
                ...
                helper.visualizeTransitions(sess, input_dict_func(batch), generative_dict, save_dir=global_args.exp_dir+'Visualization/Train/', postfix='train_'+str(epoch))
    ...
    def test(epoch):
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            test_generator_cost_np, test_discriminator_cost_np = sess.run([test_outs_dict['geo_generator_cost'], test_outs_dict['discriminator_cost']], 
                feed_dict = input_dict_func(batch, np.asarray([0,])))            
        ...
        if not (data_loader.__module__ == 'datasetLoaders.RandomManifoldDataLoader' or data_loader.__module__ == 'datasetLoaders.ToyDataLoader'):
            ...
            helper.visualizeTransitions(sess, input_dict_func(batch), generative_dict, save_dir=global_args.exp_dir+'Visualization/Test/', postfix='test_'+str(epoch))
