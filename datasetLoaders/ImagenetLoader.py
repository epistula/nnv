...
class DataLoader:
	def __init__(self, batch_size, time_steps, cuda = False, image_mode='cubic', image_size = 64):
		...
		self.dataset_path = '/home/mcgemici/datasets/imagenet_data/imagenet_'+str(image_size)+'/'
		self._batch_observed_data_image = np.zeros((self.batch_size, self.time_steps, self.image_size, self.image_size, 3), np.float32)

		self.train_path = self.dataset_path+'train/*.png'
		self.test_path = self.dataset_path+'test/*.png'
		
		self.train_files = glob.glob(self.train_path)
		self.test_files = glob.glob(self.test_path)

		self.n_train_examples = min(len(self.train_files), 200000)
		self.n_test_examples = min(len(self.test_files), 50000)

		self.train_files = self.train_files[:self.n_train_examples]
		self.test_files = self.test_files[self.n_train_examples:]
		...
		try: 
			...
			self.train_data = np.load(self.dataset_path+'train/train.npy')
			...
			np.save(self.dataset_path+'train/train.npy', self.train_data)
		
		try: 
			assert(not reset_data)
			print('Trying to load from processed test file.')
			self.test_data = np.load(self.dataset_path+'test/test.npy')
			...
			np.save(self.dataset_path+'test/test.npy', self.test_data)

		# self.train_data = self.normalize(self.train_data) 
		# self.test_data = self.normalize(self.test_data) 
		
		end = time.time()
		print('Loaded imagenet data. Time: ', (end-start))
