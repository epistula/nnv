...
if dataset_to_use == 'FLOWERS':
    # # # FLOWERS
    parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
    parser.add_argument('--global_exp_dir', type=str, default='./experimentsFLOWERS', help='Directory to put the experiments.')
    ...
    parser.add_argument('--gradient_clipping', type=float, default=500, help='Initial weight decay.')
    ...
    parser.add_argument('--latent_vis_epoch_rate', type=list, default=[200,1], help='latent epoch repeat')
    parser.add_argument('--reconst_vis_epoch_rate', type=list, default=[50,1], help='reconst epoch repeat')
    parser.add_argument('--interpolate_vis_epoch_rate', type=list, default=[50,0], help='interpolation epoch repeat')
    ...
    # # CUB
    parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
    parser.add_argument('--global_exp_dir', type=str, default='./experimentsCUB', help='Directory to put the experiments.')
    parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
    parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
    parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
    parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
    ...
    # # CELEB-A
    parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
    parser.add_argument('--global_exp_dir', type=str, default='./experimentsCELEB', help='Directory to put the experiments.')
    parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
    parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
    parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
    parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
    ...
    parser.add_argument('--latent_vis_epoch_rate', type=list, default=[20,1], help='latent epoch repeat')
    parser.add_argument('--reconst_vis_epoch_rate', type=list, default=[1,0], help='reconst epoch repeat')
    parser.add_argument('--interpolate_vis_epoch_rate', type=list, default=[1,0], help='interpolation epoch repeat')
    parser.add_argument('--inception_score_epoch_rate', type=list, default=[0,1], help='compute inception score')
    ...
    # # # TOY
    parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
    parser.add_argument('--global_exp_dir', type=str, default='./experimentsTOY', help='Directory to put the experiments.')
    parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
    parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
    parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
    parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
...
with tf.Graph().as_default():
    ...
    with tf.variable_scope("training"):
        ...
        discriminator_vars = [v for v in tf.trainable_variables() if 'Discriminator' in v.name or 'TransportCostFeatureMap' in v.name]
        generator_vars = [v for v in tf.trainable_variables() if 'Decoder' in v.name] 
        encoder_vars = [v for v in tf.trainable_variables() if 'TransportPlan' in v.name or 'EncodingPlan' in v.name or 'Encoder' in v.name or 'MixingPlan' in v.name] 

        # Weight clipping
        if len(discriminator_vars)>0:
            ...
            clip_op_list = []
            for e in discriminator_vars: clip_op_list.append(tf.assign(e, tf.clip_by_value(e, -0.01, 0.01)))

    generator_step_tf, discriminator_step_tf, encoder_step_tf = None, None, None 
    if global_args.optimizer_class == 'Adam':
        if data_loader.__module__ == 'datasetLoaders.RandomManifoldDataLoader' or data_loader.__module__ == 'datasetLoaders.ToyDataLoader':
            if model.generator_cost is not None:
                generator_step_tf = helper.clipped_optimizer_minimize(optimizer=tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.5, beta2=0.999, epsilon=1e-08), 
                                          loss=model.generator_cost, var_list=generator_vars, global_step=global_step, clip_param=global_args.gradient_clipping)
            if model.discriminator_cost is not None:
                discriminator_step_tf = helper.clipped_optimizer_minimize(optimizer=tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.5, beta2=0.999, epsilon=1e-08), 
            ...
            if model.encoder_cost is not None:
                encoder_step_tf = helper.clipped_optimizer_minimize(optimizer=tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.5, beta2=0.999, epsilon=1e-08), 
    ...
    if model.generator_cost is None: model.generator_cost = tf.zeros(shape=[1,])[0]
    if model.discriminator_cost is None: model.discriminator_cost = tf.zeros(shape=[1,])[0]
    if model.encoder_cost is None: model.encoder_cost = tf.zeros(shape=[1,])[0]

    helper.variable_summaries(model.generator_cost, '/generator_cost')
    helper.variable_summaries(model.discriminator_cost, '/discriminator_cost')
    helper.variable_summaries(model.encoder_cost, '/encoder_cost')
    ...
    if global_args.inception_score_epoch_rate[0]>0:
    ...
    def train(epoch):
        ...
        report_count = 0
        data_loader.train()
        train_trans_loss_accum, train_gen_loss_accum, train_dis_loss_accum, train_likelihood_accum, train_kl_accum, batch_size_accum = 0, 0, 0, 0, 0, 0
        start = time.time();
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            if trans_bool and encoder_step_tf is not None:
                # np_encoder_step = sess.run(encoder_step_tf, feed_dict = curr_feed_dict)
                trans_steps = trans_steps+1
                
            if disc_bool and discriminator_step_tf is not None:
                # np_disc_step = sess.run(discriminator_step_tf, feed_dict = curr_feed_dict)
                disc_steps = disc_steps+1

            if gen_bool and generator_step_tf is not None:
                np_gen_step, np_encoder_step, np_encoder_cost, np_generator_cost, np_discriminator_cost = \
                sess.run([generator_step_tf, encoder_step_tf, model.encoder_cost, model.generator_cost,
                model.discriminator_cost], feed_dict = curr_feed_dict)
                gen_steps = gen_steps+1


            train_trans_loss_accum += curr_batch_size*np_encoder_cost
            train_gen_loss_accum += curr_batch_size*np_generator_cost
            train_dis_loss_accum += curr_batch_size*np_discriminator_cost
            batch_size_accum += curr_batch_size

            if batch_idx % global_args.log_interval == 0:
                report_count = report_count+1
                np_encoder_cost, np_generator_cost, np_discriminator_cost, np_encoder_reg_cost, np_disc_reg_cost, np_mean_critic_real, \
                np_mean_critic_gen, np_mean_critic_reg, np_mean_OT_primal, np_mean_OT_dual = \
                sess.run([model.encoder_cost, model.generator_cost, model.discriminator_cost, model.encoder_reg_cost,
                          model.disc_reg_cost, model.mean_critic_real, model.mean_critic_gen, model.mean_critic_reg, 
                          model.mean_OT_primal, model.mean_OT_dual], \
                feed_dict = curr_feed_dict)

                # np_mean_neg_ent_posterior, np_mean_neg_cross_ent_posterior, np_mean_kl_posterior_prior = \
                # sess.run([model.mean_neg_ent_posterior, model.mean_neg_cross_ent_posterior, model.mean_kl_posterior_prior], 
                # feed_dict = curr_feed_dict)
                
                end = time.time();
                # report_format = 'Train: Epoch {} [{:7d}] Time {:.2f} Generator Cost {:.2f} Discriminator Cost {:.2f} Penalty cost {:.2f} ' +\
                #                 'C-real {:.2f} C-reg {:.2f} C-gen {:.2f}  t {:2d} d {:2d} g {:2d} ne_post {:.1f} nce_post_prior {:.1f}  kl {:.2f}'

                report_format = 'Train: Epoch {} [{:7d}] Time {:.2f} Encoder Cost {:.2f} Generator Cost {:.2f} Discriminator Cost {:.2f} ' +\
                                'Enc_Reg cost {:.2f} Disc_Reg cost {:.2f} C-real {:.2f} C-reg {:.2f} C-gen {:.2f}  t {:2d} d {:2d} g {:2d}'

                print(report_format.format(epoch, batch_idx * curr_batch_size, (end - start), \
                      np_encoder_cost, np_generator_cost, np_discriminator_cost, np_encoder_reg_cost, np_disc_reg_cost, 
                      np_mean_critic_real, np_mean_critic_reg, np_mean_critic_gen, trans_steps, disc_steps, gen_steps))

                with open(global_args.exp_dir+"training_traces.txt", "a") as text_file:
                    text_file.write(str(np_encoder_cost) + ', ' + \
                                    str(np_generator_cost) + ', ' + \
                                    str(np_discriminator_cost) + '\n')
                with open(global_args.exp_dir+"training_primal_dual.txt", "a") as text_file:
                    text_file.write(str(np_mean_OT_primal) + ', ' + \
                                    str(np_mean_OT_dual) + ', ' + '\n')
                start = time.time()
        ...
        train_trans_loss_accum /= batch_size_accum
        ...
        if epoch % global_args.vis_interval == 0:
            print('====> Average Train: Epoch: {}\tEncoder Cost {:.2f}\tGenerator Cost: {:.3f}\tDiscriminator Cost: {:.3f}'.format(
                  epoch, train_trans_loss_accum, train_gen_loss_accum, train_dis_loss_accum))
    ...
    def test(epoch):
        global random_batch_data, fixed_batch_data
        data_loader.eval()
        test_trans_loss_accum, test_gen_loss_accum, test_dis_loss_accum, test_batch_size_accum, test_mean_OT_primal_accum, test_mean_OT_dual_accum = 0, 0, 0, 0, 0, 0
        start = time.time()
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            test_np_encoder_cost, test_np_generator_cost, test_np_discriminator_cost, test_np_mean_OT_primal, test_np_mean_OT_dual = \
            sess.run([model.encoder_cost, model.generator_cost, model.discriminator_cost, model.mean_OT_primal, model.mean_OT_dual], 
            feed_dict = curr_feed_dict)

            test_trans_loss_accum += curr_batch_size*test_np_encoder_cost
            test_gen_loss_accum += curr_batch_size*test_np_generator_cost
            test_dis_loss_accum += curr_batch_size*test_np_discriminator_cost
        ...
        test_trans_loss_accum /= test_batch_size_accum
        test_gen_loss_accum /= test_batch_size_accum
        test_dis_loss_accum /= test_batch_size_accum
        test_mean_OT_primal_accum /= test_batch_size_accum
        test_mean_OT_dual_accum /= test_batch_size_accum

        end = time.time();
        print('====> Average Test: Epoch {} Time {:.2f} Encoder Cost {:.2f} Generator Cost {:.2f} Discriminator Cost {:.2f}'.format(
              epoch, (end - start), test_trans_loss_accum, test_gen_loss_accum, test_dis_loss_accum))

        with open(global_args.exp_dir+"test_traces.txt", "a") as text_file:
            text_file.write(str(test_trans_loss_accum) + ', ' + \
                            str(test_gen_loss_accum) + ', ' + \
                            str(test_dis_loss_accum) + ', ' + '\n')
        with open(global_args.exp_dir+"test_primal_dual.txt", "a") as text_file:
            text_file.write(str(test_mean_OT_primal_accum) + ', ' + \
                            str(test_mean_OT_dual_accum) + ', ' + '\n')
    ...
    def visualize(epoch):
        ...
        if global_args.interpolate_vis_epoch_rate[0]>0 and global_args.curr_epoch % global_args.interpolate_vis_epoch_rate[0] == global_args.interpolate_vis_epoch_rate[1]: 
            ...
            save_dir=global_args.exp_dir+'Visualization/test_real_sample_interpolated/', postfix = '_test_real_sample_interpolated'+str(epoch))
        ...
        if global_args.latent_vis_epoch_rate[0]>0 and global_args.curr_epoch % global_args.latent_vis_epoch_rate[0] == global_args.latent_vis_epoch_rate[1]: 
            ...
            inverse_chosen_indeces2 = np.zeros((all_tsne_input.shape[0],), dtype=int)
            for ind, e in enumerate(chosen_indeces2): inverse_chosen_indeces2[e] = ind
            all_tsne_scrambled = TSNE().fit_transform(all_tsne_input[chosen_indeces2, :])
            ...
            helper.dataset_plotter(class_wise, point_thickness = 4, save_dir = global_args.exp_dir+'Visualization/z_projection_posterior/', postfix = '_z_projection_posterior'+str(epoch), postfix2 = 'z_projection_posterior')
            helper.dataset_plotter([all_tsne_normalized_prior,], point_thickness = 4, save_dir = global_args.exp_dir+'Visualization/z_projection_prior/', postfix = '_z_projection_prior'+str(epoch), postfix2 = 'z_projection_prior')
            helper.dataset_plotter([all_tsne_normalized_prior, all_tsne_normalized_posterior], point_thickness = 4, save_dir = global_args.exp_dir+'Visualization/z_projection_prior_posterior/', postfix = '_z_projection_prior_posterior'+str(epoch), postfix2 = 'z_projection_prior_posterior')
    ...
    #         if global_args.curr_epoch % global_args.latent_vis_epoch_rate[0] == global_args.latent_vis_epoch_rate[1]: 
    #             np_posterior_latent_code, np_prior_latent_code = sess.run([model.posterior_latent_code, model.prior_latent_code], feed_dict = curr_feed_dict)
    #             if all_np_posterior_latent_code is None: all_np_posterior_latent_code = np_posterior_latent_code
    #             else: all_np_posterior_latent_code = np.concatenate([all_np_posterior_latent_code, np_posterior_latent_code], axis=0)
    #             if all_np_prior_latent_code is None: all_np_prior_latent_code = np_prior_latent_code
