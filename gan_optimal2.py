...
with tf.Graph().as_default():
    ...
    def train(epoch):
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            trans_train_step_np = sess.run([train_transport_step_tf], feed_dict = input_dict_func(batch, np.asarray([0,])))
            ...
            if batch_idx % global_args.log_interval == 0:
                ...
                print('Train: Epoch {} [{:7d} ()]\tGenerator Cost: {:.6f}\tDiscriminator Cost: {:.6f}\tTime: {:.3f}, variational cost {:.3f}, transport cost {:.3f}'.format(
                      epoch, batch_idx * curr_batch_size, generator_cost_np, discriminator_cost_np, (end - start), variational_cost_np, mean_transport_cost_np))
