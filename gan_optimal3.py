...
parser.add_argument('--global_exp_dir', type=str, default='./experimentsOPTIMALFC', help='Directory to put the experiments.')
...
with tf.Graph().as_default():
    ...
    if global_args.optimizer_class == 'RmsProp':
        ...
        train_transport_step_tf = tf.train.AdamOptimizer(learning_rate=0.0001, 
            beta1=0.5, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['variational_cost'], var_list=transport_vars, global_step=global_step)
    ...
    def train(epoch):
        ...
        prev_mean_transport_cost_np = None
        prev_variational_cost_np = None
        last_model_variational_cost_np = None
        turn = 'gen'
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            if turn == 'trans' or turn == 'disc' or turn == 'gen':
                ...
                if prev_mean_transport_cost_np is None: prev_mean_transport_cost_np = mean_transport_cost_np
                if prev_variational_cost_np is None: prev_variational_cost_np = variational_cost_np
            ...
            if turn == 'gen':
                ...
                sess.run([train_generator_step_tf, train_outs_dict['generator_cost'], train_outs_dict['discriminator_cost'], 
                      ...
                      feed_dict = input_dict_func(batch, np.asarray([0,])))
                if last_model_variational_cost_np is None: last_model_variational_cost_np = variational_cost_np
                last_model_variational_cost_np = variational_cost_np
            ...
            #     else:
            #         if np.abs(variational_cost_np-prev_variational_cost_np) < 0.1: turn = 'disc'
            # elif turn == 'disc':
            ...
            if turn == 'trans':
                if trans_steps % (critic_rate*generator_rate) == 0 or np.abs(variational_cost_np-prev_variational_cost_np) < 0.01: turn = 'disc'
            elif turn == 'disc':
                if disc_steps % generator_rate == 0: turn = 'gen'
            elif turn == 'gen':
                if gen_steps % 1 == 0: turn = 'trans'
