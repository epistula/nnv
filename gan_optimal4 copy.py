...
with tf.Graph().as_default():
    ...
    if global_args.optimizer_class == 'RmsProp':
        ...
        train_transport_step_tf = tf.train.AdamOptimizer(learning_rate=0.0001, 
            beta1=0.2, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['variational_cost'], var_list=transport_vars, global_step=global_step)
    ...
    def train(epoch):
        critic_rate, generator_rate = 1, 4
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            if turn == 'trans':
                if epoch<3: 
                    if trans_steps % (10*generator_rate) == 0: turn = 'disc'
                else:
