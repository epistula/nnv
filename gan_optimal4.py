...
# # MNIST
parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
parser.add_argument('--global_exp_dir', type=str, default='./experimentsOPTIMALFC2', help='Directory to put the experiments.')
parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
...
# # # # TOY
# parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
# parser.add_argument('--global_exp_dir', type=str, default='./experimentsGanTests557', help='Directory to put the experiments.')
# parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
# parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
# parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
# parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
...
# parser.add_argument('--n_encoder', type=int, default=200, help='n_encoder.')
# parser.add_argument('--n_decoder', type=int, default=200, help='n_decoder.')
# parser.add_argument('--n_context', type=int, default=1, help='n_context.')
# parser.add_argument('--n_state', type=int, default=1, help='n_state.')
# parser.add_argument('--n_latent', type=int, default=10, help='n_latent.')
# global_args = parser.parse_args()
# global_args.curr_epoch = 1
...
from models.OPTIMAL2.Model import Model
# from models.OPTIMAL2.ModelReverse import Model
...
with tf.Graph().as_default():
    ...
    with tf.variable_scope("training"):
        ...
        transport_vars = [v for v in tf.trainable_variables() if 'TransportPlan' in v.name] 
    ...
    if global_args.optimizer_class == 'RmsProp':
        ...
        train_generator_step_tf = tf.train.AdamOptimizer(learning_rate=0.0001, 
            beta1=0.9, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['generator_cost'], var_list=generator_vars, global_step=global_step)
        ...
        train_transport_step_tf = tf.train.AdamOptimizer(learning_rate=0.0001, 
            beta1=0.1, beta2=0.999, epsilon=1e-08).minimize(train_outs_dict['variational_cost'], var_list=transport_vars, global_step=global_step)
    ...
    helper.variable_summaries(train_outs_dict['variational_cost'], '/variational_cost')
    ...
    def train(epoch):
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            if turn == 'trans' or turn == 'disc' or turn == 'gen':
                trans_train_step_np, variational_cost_np, mean_transport_cost_np = \
                sess.run([train_transport_step_tf, train_outs_dict['variational_cost'], train_outs_dict['mean_transport_cost']], 
                feed_dict = input_dict_func(batch, np.asarray([epoch,])))
                trans_steps = trans_steps+1

            if turn == 'disc' or turn == 'gen':
                disc_train_step_np, variational_cost_np, mean_transport_cost_np = \
                sess.run([train_discriminator_step_tf, train_outs_dict['variational_cost'], train_outs_dict['mean_transport_cost']],
                feed_dict = input_dict_func(batch, np.asarray([epoch,])))
            ...
            if batch_idx % global_args.log_interval == 0:
                ...
                # print('Train: Epoch {} [{:7d} ()]\tGenerator Cost: {:.6f}\tDiscriminator Cost: {:.6f}\tTime: {:.3f}, variational cost {:.3f}, transport cost {:.3f}, prev transport cost {:.3f}, t {:2d} d {:2d} g {:2d}, t/d {:.1f}, t/g {:.1f}'.format(
                #       epoch, batch_idx * curr_batch_size, generator_cost_np, discriminator_cost_np, (end - start), variational_cost_np, mean_transport_cost_np, last_model_variational_cost_np, trans_steps, disc_steps, gen_steps, trans_steps/disc_steps, trans_steps/gen_steps))

                print('Train: Epoch {} [{:7d} ()]\tGenerator Cost: {:.6f}\tDiscriminator Cost: {:.6f}\tTime: {:.3f}, variational cost {:.3f}, transport cost {:.3f}, t {:2d} d {:2d} g {:2d}, t/d {:.1f}, t/g {:.1f}'.format(
                      epoch, batch_idx * curr_batch_size, generator_cost_np, discriminator_cost_np, (end - start), variational_cost_np, mean_transport_cost_np, trans_steps, disc_steps, gen_steps, trans_steps/disc_steps, trans_steps/gen_steps))
            ...
            # prev_variational_cost_np = variational_cost_np

        summary_str = sess.run(merged_summaries, feed_dict = input_dict_func(batch, np.asarray([epoch,])))
        ...
        if epoch % checkpoint_time == 0:
            ...
            if data_loader.__module__ == 'datasetLoaders.RandomManifoldDataLoader' or data_loader.__module__ == 'datasetLoaders.ToyDataLoader':
                ...
                disc_cost_real_np = sess.run(train_outs_dict['critic_real'], feed_dict = input_dict_func(batch, np.asarray([epoch,])))
                ...
                distributions.visualizeProductDistribution3(sess, input_dict_func(batch), batch, inference_obs_dist, transport_dist, generative_dict['obs_dist'], 
    ...
    while global_args.curr_epoch < global_args.epochs + 1:
        ...
        global_args.curr_epoch += 1
            ...
            # gen_train_step_np, generator_cost_np, discriminator_cost_np, variational_cost_np, mean_transport_cost_np = 0, 0, 0, 0, 0
