...
parser.add_argument('--global_exp_dir', type=str, default='./experimentsCELEB-02', help='Directory to put the experiments.')
...
# # # MNIST
# parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
# parser.add_argument('--global_exp_dir', type=str, default='./experimentsDAEGAN456', help='Directory to put the experiments.')
# parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
# parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
# parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
# parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
...
# parser.add_argument('--n_context', type=int, default=1, help='n_context.')
# parser.add_argument('--n_state', type=int, default=1, help='n_state.')
# parser.add_argument('--n_latent', type=int, default=784, help='n_latent.')
# global_args = parser.parse_args()
# global_args.curr_epoch = 1
...
# # # # TOY
# parser = argparse.ArgumentParser(description='Tensorflow Gan Models')
# parser.add_argument('--global_exp_dir', type=str, default='./experimentsGanTests669', help='Directory to put the experiments.')
# parser.add_argument('--exp_dir_postfix', type=str, default='', help='Directory to put the experiment postfix.')
# parser.add_argument('--dataset_dir', type=str, default='../dataset/dataset_both2/scripted/*.npz', help='Directory of data.')
# parser.add_argument('--restore_dir', type=str, default='/7cb1611a0a584269a019dc8342aee213/checkpoint/', help='Directory of restore experiment.')
# parser.add_argument('--restore', type=bool, default=False, help='Restore model.')
...
from models.FLOWGAN.Model import Model
# from models.WGANGP2.Model import Model
...
with tf.Graph().as_default():
    ...
    #     entropy_error = (desired_entropy-train_entropy_np)
    #     modification = entropy_gain*entropy_error
    #     entropy_strength = max(min(1., entropy_strength+modification), 0.)

    def train(epoch):
        global entropy_strength, desired_entropy, entropy_gain
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            ...
            # if batch_idx == 0 or not (n_disc_steps < (disc_boost+1) or n_disc_steps % 20 !=0): 
            if batch_idx == 0 or n_disc_steps % 20 == 0: 
                gen_train_step_np, generator_cost_np, discriminator_cost_np, train_average_log_density_np, train_average_log_volume_decrease_np, train_average_volume_decrease_np = \
                    sess.run([train_generator_step_tf, train_outs_dict['generator_cost'], train_outs_dict['discriminator_cost'], 
                              train_outs_dict['average_log_density'], train_outs_dict['average_log_volume_decrease'], train_outs_dict['average_volume_decrease']],
                              feed_dict = input_dict_func(batch, np.asarray([entropy_gain,])))

                n_gen_steps += 1
                # if n_gen_steps % 500 == 0: disc_boost = n_disc_steps
            ...
            if batch_idx % global_args.log_interval == 0:
                ...
                print('Train: Epoch {} [{:7d} ()]\tGen Cost: {:.4f}\tDisc Cost: {:.4f}\tTime: {:.3f}, Max disc weight {:.4f}, density {:.3f}, log volume decrease {:.3f}, volume decrease {:.3f}'.format(
                      epoch, batch_idx * curr_batch_size, generator_cost_np, discriminator_cost_np, (end - start), max_discriminator_weight, train_average_log_density_np, train_average_log_volume_decrease_np, train_average_volume_decrease_np))
            ...
            if batch_idx % 5*global_args.log_interval == 0:
                helper.visualize_images_from_dists(sess, input_dict_func(batch), batch, model.obs_sample, generative_dict['obs_sample_out'], 
        ...
        if epoch % checkpoint_time == 0:
            ...
            if data_loader.__module__ == 'datasetLoaders.RandomManifoldDataLoader' or data_loader.__module__ == 'datasetLoaders.ToyDataLoader':
                ...
                if data_loader.n_dims == 2:
                    xmin, xmax, ymin, ymax, X_dense, Y_dense = -1.5, 1.5, -1.5, 1.5, 250, 250
    ...
    def test(epoch):
        ...
        for batch_idx, curr_batch_size, batch in data_loader: 
            test_generator_cost_np, test_discriminator_cost_np, test_average_log_density_np, test_average_log_volume_decrease_np, test_average_volume_decrease_np = \
                sess.run([test_outs_dict['generator_cost'], test_outs_dict['discriminator_cost'], 
                          test_outs_dict['average_log_density'], test_outs_dict['average_log_volume_decrease'], test_outs_dict['average_volume_decrease']], 
                feed_dict = input_dict_func(batch, np.asarray([entropy_gain,])))            
        ...
        print('\n\n====> Test: Epoch {}\tGen Cost: {:.4f}\tDisc Cost: {:.4f}\tTime: {:.3f}, density {:.3f}, log volume decrease {:.3f}, volume decrease {:.3f}\n'.format(
              epoch, test_gen_loss_accum/batch_size_accum, test_dis_loss_accum/batch_size_accum, (end - start), test_average_log_density_np, test_average_log_volume_decrease_np, test_average_volume_decrease_np))
    ...
    while global_args.curr_epoch < global_args.epochs + 1:
        ...
        global_args.curr_epoch += 1
                ...
                #     sess.run([train_generator_step_tf, train_outs_dict['generator_cost'], train_outs_dict['discriminator_cost']],
                #               feed_dict = input_dict_func(batch, np.asarray([entropy_gain,])))
                # train_average_log_density_np, train_average_log_volume_decrease_np, train_average_volume_decrease_np = 0, 0, 0

            # disc_train_step_np = 0
            # gen_train_step_np, generator_cost_np, discriminator_cost_np, train_average_log_density_np, train_average_log_volume_decrease_np, train_average_volume_decrease_np = 0, 0, 0, 0, 0, 0
